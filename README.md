...
## 目的
- ROS 2 Humble 上で 複数種類のカメラ入力（Webカメラ / Xtion / RealSense）を Docker Compose で運用する
...
### 4-2. Webカメラの camera_info を読み込む
Webカメラは webcam.launch.py で camera_info_url を指定し、yamlを読み込みます。
...
- launch/*.launch.py に DeclareLaunchArgument を用意
...
### 6-2. RealSenseのトピック名が違う
RealSenseは環境でトピック名が変わることがあります。

以下で実際の名前を確認し、realsense.launch.py の引数を合わせてください。
```
ros2 topic list | grep realsense
```

## 7. 推奨運用（下流のYOLO等）
- 下流は /camera/image_raw（必要ならDepth系も）だけ購読
- カメラの切替は relay で吸収
